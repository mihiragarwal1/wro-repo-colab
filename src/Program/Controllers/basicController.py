...
# def drive(img):
    ...
    leftEdgesImg, gLeftImg, rLeftImg = preProcessingController.filter(preProcessingController.undistort(read[0]))
    rightEdgesImg, gRightImg, rRightImg = preProcessingController.filter(preProcessingController.undistort(read[1]))
    # print("filter + undistort: ", time.perf_counter() - start)
    start = time.perf_counter()
    # leftCoordinates, rightCoordinates = preProcessingController.getDistances(leftEdgesImg, rightEdgesImg)

    leftHeights, rightHeights, leftWallStarts, rightWallStarts = preProcessingController.getRawHeights(leftEdgesImg, rightEdgesImg)
    rLeftContours = preProcessingController.getContours(rLeftImg)
    gLeftContours = preProcessingController.getContours(gLeftImg)
    rRightContours = preProcessingController.getContours(rRightImg)
    gRightContours = preProcessingController.getContours(gRightImg)

    rawLeftWalls = preProcessingController.getWalls(leftHeights.copy(), rLeftContours, gLeftContours)
    rawRightWalls = preProcessingController.getWalls(rightHeights.copy(), rRightContours, gRightContours)

    rContours = preProcessingController.mergeContours(rLeftContours, rRightContours, leftHeights, rightHeights)
    gContours = preProcessingController.mergeContours(gLeftContours, gRightContours, leftHeights, rightHeights)

    corners, walls = preProcessingController.processWalls(rawLeftWalls, rawRightWalls)
    ...
    if slam.carDirectionGuesses < 9:
        #majority vote
        oneFourths = int((preProcessingController.imageWidth - 3) / 4)
        threeFourths = int((preProcessingController.imageWidth - 3) * 3 / 4)
