...
# def drive(img):
    ...
    leftEdgesImg, gLeftImg, rLeftImg = preProcessingController.filter(preProcessingController.undistort(imgs[0]))
    rightEdgesImg, gRightImg, rRightImg = preProcessingController.filter(preProcessingController.undistort(imgs[1]))
    # leftCoordinates, rightCoordinates = preProcessingController.getDistances(leftEdgesImg, rightEdgesImg) # OOOOOOOOOOOOOOOF BORKEN
    leftHeights, rightHeights = preProcessingController.getRawHeights(leftEdgesImg, rightEdgesImg)
    rLeftContours, gLeftContours, rRightContours, gRightContours = preProcessingController.getContours(rLeftImg, gLeftImg, rRightImg, gRightImg)
    # leftWalls = preProcessingController.getWallLandmarks(leftCoordinates, rLeftContours, gLeftContours)
    # rightWalls = preProcessingController.getWallLandmarks(rightCoordinates, rRightContours, gRightContours)
    leftWalls = preProcessingController.getWalls(leftHeights.copy(), rLeftContours, gLeftContours)
    rightWalls = preProcessingController.getWalls(rightHeights.copy(), rRightContours, gRightContours)
    rContours = []
    for contour in rLeftContours:
        rContours.append(preProcessingController.getRawDistance(contour[0], leftHeights[contour[0]], -1))
    for contour in rRightContours:
        rContours.append(preProcessingController.getRawDistance(contour[0], rightHeights[contour[0]], 1))
    gContours = []
    for contour in gLeftContours:
        gContours.append(preProcessingController.getRawDistance(contour[0], leftHeights[contour[0]], -1))
    for contour in gRightContours:
        gContours.append(preProcessingController.getRawDistance(contour[0], rightHeights[contour[0]], 1))
    corners, walls = preProcessingController.processWalls(leftWalls, rightWalls)
    ...
    # read[0] = preProcessingController.undistort(read[0])
    # for l in leftWalls:
...
def getSteering(leftHeights, rightHeights, rLeftContours, gLeftContours, rRightContours, gRightContours):
    ...
    if len(landmarks) == 4:
        ...
        if rightHeights[preProcessingController.imageWidth - 1 - 56] > sideWallThreshold:
            steering += -80
        if rightHeights[preProcessingController.imageWidth - 1 - 380] > frontWallThreshold:
            steering += -slam.carDirection * 80
